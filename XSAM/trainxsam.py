...
from utils import estimate_largest_eigenvector, modify_gradient_with_projection ,modify_gradientpho_with_projection
...
def parse_args():
    ...
    #parser.add_argument("--resnet_version", type=int, choices=[18, 34, 50, 101, 152], default="help="Specify which ResNet version to use (18, 34, 50, 101, 152)")
    parser.add_argument("--model", type=str, choices=["resnet", "vgg", "densenet", "alexnet","wideresnet28","pyramidnet"], default="alexnet")
    ...
    parser.add_argument("--sam", type=str, default="SGD", help="Which SAM mode to use: [SGD, SAM, X-SAM]")
    parser.add_argument("--alpha", type=float, default=0.2, help="Alpha parameter for Eigen-SAM")
    parser.add_argument("--freq", type=int, default=5, help="Frequency for updating the largest eigenvector")
    ...
    for epoch in range(epochs):
        ...
        for step, (images, labels) in enumerate(train_loader):
            ...
            if args.sam == 'SAM':
                ...
                if v is not None:
                    modify_gradientpho_with_projection(model, v, alpha=args.alpha)
                optimizer.second_step(zero_grad=True)
...
def build_model(args, device, ckpt=None):
    ...
    model_dict = {
        'resnet': lambda: ResNet34(num_classes=num_classes),
        'densenet': lambda: DenseNet121(num_classes=num_classes),
        'vgg': lambda: vgg11_bn(num_classes=num_classes),  # 固定用 vgg11_bn
        'alexnet': lambda: AlexNet(num_classes=num_classes),
        'wideresnet28': lambda: WideResNet28(num_classes=num_classes),
        'pyramidnet': lambda: PyramidNet(
